<template>
  <div id="baiduSug">
    <div class="logo"></div>
    <input type="text" v-model.trim="t1" @keyup="get($event)" placeholder="请输入需要查询的信息" 
      @keydown.38="upSelect()" @keydown.40="downSelect()" @keydown.enter="t1=myData[selected].oldValue,selected=-1,showList=false" 
      @focus="showList=true" />
    <input type="button" name="" id="" value="百度一下" @click="openLink()" />
    <ul v-show="showList">
      <li v-for="(value,$index) in myData" :class="{select:$index==selected}" v-html="value.newValue" @click.left="clickItem(value)"
